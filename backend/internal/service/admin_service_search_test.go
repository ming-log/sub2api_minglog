...
func (s *accountRepoStubForAdminList) ListWithFilters(_ context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, accountPlanType string) ([]Account, *pagination.PaginationResult, error) {
	...
	s.listWithFiltersPlanType = accountPlanType
...
func TestAdminService_ListAccounts_WithAccountPlanType(t *testing.T) {
	t.Run("account_plan_type 参数正常传递到 repository 层", func(t *testing.T) {
		repo := &accountRepoStubForAdminList{
